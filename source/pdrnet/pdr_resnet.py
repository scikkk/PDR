...
from source.pdrnet.dataset import MyDataset
from IPython import embed

from pdrnet.model_resnet1d import *


_input_channel, _output_channel = 12, 3
...
def run_test(network, data_loader, device, eval_mode=True):
    ...
    if eval_mode:
        network.eval()
    for feat, targ in data_loader:
        feat = feat.view(-1, _input_channel, 1)
...
def add_summary(writer, loss, step, mode):
    names = '{0}_loss/loss_x,{0}_loss/loss_y,{0}_loss/loss_z,{0}_loss/loss_sin,{0}_loss/loss_cos'.format(mode).split(',')
...
def get_dataset(root_dir, data_list, **kwargs):
    ...
    dataset = MyDataset(root_dir, data_list, _input_channel, _output_channel, shuffle=shuffle)
    return dataset


def get_dataset_from_list(root_dir, list_path):
    with open(list_path) as f:
        data_list = [s.strip().split(',' or ' ')[0] for s in f.readlines() if len(s) > 0 and s[0] != '#']
    return get_dataset(root_dir, data_list, mode='train')


def train(args, **kwargs):
    # Loading data
    start_t = time.time()
    train_dataset = get_dataset_from_list(args.root_dir, args.train_list)
    ...
    try:
        for epoch in range(start_epoch, args.epochs):
            ...
            for batch_id, (feat, targ) in enumerate(train_loader):
...
def test_sequence(args):
    ...
    for data in test_data_list:
        ...
        print('Sequence {}, loss {} / {}, ate {:.6f}, rte {:.6f}'.format(data, losses, np.mean(losses)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_list', type=str,default='../lists/data_list.txt')
    parser.add_argument('--val_list', type=str, default=None)
    parser.add_argument('--test_list', type=str, default=None)
    parser.add_argument('--test_path', type=str, default=None)
    parser.add_argument('--root_dir', type=str, default='../data/processed/', help='Path to data directory')
    ...
    parser.add_argument('--out_dir', type=str, default='PDRNet-out')
    ...
    # args = get_args_from_json('./config/heading_model_defaults.json', args_dict)  # modify config json file
